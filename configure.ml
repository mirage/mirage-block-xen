...
let configure verbose =
  let xenctrl  = find_ocamlfind verbose "xenctrl" in
  let xenctrl_lwt = find_ocamlfind verbose "xenctrl.lwt" in
  (* Write config.mk *)
  let lines = 
    [ "# Warning - this file is autogenerated by the configure script";
      "# Do not edit";
      (* The userspace Gnt.get_n function is missing: *)
      Printf.sprintf "ENABLE_BLKFRONT=--%s-blkfront" (if xenctrl && not xenctrl_lwt then "enable" else "disable");
      (* The backend is more portable: *)
      Printf.sprintf "ENABLE_BLKBACK=--%s-blkback" (if xenctrl then "enable" else "disable");
    ] in
